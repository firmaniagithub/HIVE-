...
payr1=hv.table('payr.PAYR_1')
mtd=hv.table('payr.mtd')\
    .select('subs_id','lacci_id').withColumnRenamed("subs_id","subs_id_mtd").withColumnRenamed("lacci_id","lacci_id_mtd")\
    .dropDuplicates()

join_mtd = payr1.join(mtd,payr1.subs_id ==  mtd.subs_id_mtd, how = 'left')\
    .withColumn("trx_lacci_final",when(F.col('trx_lacci') == "|",""))\
    .withColumn("trx_fix" ,when((F.col('trx_lacci_final').isNull()) & (F.col('subs_id') == F.col('subs_id_mtd')), (F.col('lacci_id_mtd')))\
        ...
        .withColumnRenamed("trx_fix","trx_lacci")
        
mm=hv.table('payr.mm')\
    .select('subs_id','lacci_id').withColumnRenamed("subs_id","subs_id_mm").withColumnRenamed("lacci_id","lacci_id_mm")\
    .dropDuplicates(['subs_id_mm'])
join_mm = join_mtd.join(mm,join_mtd.subs_id ==  mm.subs_id_mm, how = 'left')\
    .withColumn("trx_lacci_final",when(F.col('trx_lacci') == "|",""))\
    .withColumn("trx_fix" ,when((F.col('trx_lacci_final').isNull()) & (F.col('subs_id') == F.col('subs_id_mm')),(F.col('lacci_id_mm')))\
        .otherwise(F.col('trx_lacci')))\
    .select('payment_timestamp','trx_date','trx_hour','msisdn','bss_order_id','plan_id','plan_name','topping_id','topping_name','plan_price','offer_id','payment_channel','cell_id','indicator_4g','future_string_1','future_string_2','future_string_3','brand','site_name','pre_post_flag','event_date','bundling_id','future_string_5','future_string_6','future_string_7','future_string_8','future_string_9','future_string_10','filename','trx_fix','retry_count','subs_id','cust_type_desc','cust_subtype_desc','subs_id_mtd','lacci_id_mtd','subs_id_mm','lacci_id_mm')\
    .withColumnRenamed("trx_fix","trx_lacci")\
    .withColumn("lacci_id_flag",when((F.col('subs_id') == F.col('subs_id_mtd')) & (F.col('lacci_id_mtd') == F.col('trx_lacci')), "1#")\
        .when ((F.col('subs_id') == F.col('subs_id_mm')) & (F.col('lacci_id_mm') == F.col('trx_lacci')), "2#").otherwise(""))\
...
lacimma=hv.table('payr.lacimma')\
    .select('lac','ci','node','provinsi','kabupaten','kecamatan','kelurahan','region_sales','branch','subbranch','address','cluster_sales','cgi_pre','cgi_post','ocs_cluster','area_sales','eci','lacci_id_lacimma')\
    .dropDuplicates(['cgi_post'])
lacimma_mm= join_mm.join(lacimma,join_mm.trx_lacci ==  lacimma.lacci_id_lacimma, how = 'left')\
    .withColumn('Flag',when(col('trx_lacci') == col('lacci_id_lacimma'),'SAME').otherwise('DIFF'))

good1=lacimma_mm.filter(col('Flag')  == 'SAME').select('payment_timestamp','trx_date','trx_hour','msisdn','bss_order_id','plan_id','plan_name','topping_id','topping_name','plan_price','offer_id','payment_channel','cell_id','indicator_4g','future_string_1','future_string_2','future_string_3','brand','site_name','pre_post_flag','event_date','bundling_id','future_string_5','future_string_6','future_string_7','future_string_8','future_string_9','future_string_10','filename','trx_lacci','retry_count','subs_id','cust_type_desc','cust_subtype_desc','lac','ci','node','provinsi','kabupaten','kecamatan','kelurahan','region_sales','branch','subbranch','cluster_sales','area_sales','lacci_id_flag','lacci_id','lacci_closing_flag','lacci_id_lacimma','eci','cgi_post')
reject1=lacimma_mm.filter(col('Flag') == 'DIFF').select('payment_timestamp','trx_date','trx_hour','msisdn','bss_order_id','plan_id','plan_name','topping_id','topping_name','plan_price','offer_id','payment_channel','cell_id','indicator_4g','future_string_1','future_string_2','future_string_3','brand','site_name','pre_post_flag','event_date','bundling_id','future_string_5','future_string_6','future_string_7','future_string_8','future_string_9','future_string_10','filename','trx_lacci','retry_count','subs_id','cust_type_desc','cust_subtype_desc','lacci_id_flag','lacci_id','lacci_closing_flag')
...
lacci_id_flag = merge.withColumn("lacci_id_flag_2",when((merge.trx_lacci != "-99") & ((merge.trx_lacci == merge.lacci_id_lacimma)\
        ...
        .withColumn("lacci_id_final",split(col("lacci_id_flag_2"), "#").getItem(1))\
        ...
        .withColumnRenamed("lacci_id_flag_2","lacci_id_flag").withColumnRenamed("lacci_id_final","lacci_id").withColumnRenamed("lacci_closing_flag_final","lacci_closing_flag") 

product_lime=hv.table('payr.product')\
    .withColumn("r_key",when ((F.col('mapping_key_type').isNull()) | (F.col('mapping_key_type') == ("")) ,(F.col('rkey')))\
        .otherwise(F.col('mapping_key_type')))\
    .select('trx_date','mapping_key_type','service_type','l1_name','l2_name','l3_name','l4_name','price','updated_date','product_name','plan_type','plan_service','quota_value','quota_unit','validity_value','validity_unit','vascode_type','file_id','load_ts','load_user','r_key')\
    .withColumnRenamed("trx_date","trx_date_product")

mm_product=lacci_id_flag.join(product_lime,lacci_id_flag.offer_id ==  product_lime.r_key, how = 'left')\
    .withColumn("file_id", lit(""))\
    .withColumn("load_ts", lit(20221219))\
    ...
    .withColumn("status_reject", when((coalesce('subs_id','cust_type_desc','cust_subtype_desc','l4_name','l3_name','l2_name','l1_name','lacci_id','lacci_closing_flag','lac','ci','node','area_sales','region_sales','branch','subbranch','cluster_sales','provinsi','kabupaten','kecamatan','kelurahan').isNull())\
